...
Telegram-монитор промоакций для новых пользователей и первого депозита. Каждые 5 минут он читает публичные источники, отправляет только новые публикации, а постоянные welcome-предложения включает в сводку в 18:00 по Тбилиси. По субботам ежедневная сводка заменяется недельной.
...
Workflow запускает poll по cron `*/5 * * * *`. Сводка запускается в `14:00 UTC`, что соответствует `18:00 Asia/Tbilisi`; скрипт сам помечает субботу недельной. GitHub cron может задерживаться на несколько минут — дедупликация не зависит от точного времени.
